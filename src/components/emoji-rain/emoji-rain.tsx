import { Component, h, Event, EventEmitter, Host, Watch } from '@stencil/core';
...
export class EmojiRain {
  dropsForDrawing: object[];
  _emoji: string[];
  _totalEmoji: number;
  _imageTransmogrifier: HTMLDivElement;
  ...
  @Watch('active')
  watchActiveHandler(newValue: boolean, oldValue: boolean) {
    console.log(`The ${oldValue} became ${newValue}`);
  }

  @Watch('useTwemoji')
  watchTwemojiHandler(newValue: boolean, oldValue: boolean) {
    console.log(`The ${oldValue} became ${newValue}`);
  }

  //events added
  @Event({
    eventName: 'emoji-rain-start',
    composed: true,
    bubbles: true,
  })
  emojiRainStart: EventEmitter<any>;

  emojiRainStartHandler() {
    this.emojiRainStart.emit();
  }

  @Event({
    eventName: 'emoji-rain-stop',
    composed: true,
    bubbles: true,
  })
  emojiRainStop: EventEmitter<any>;

  emojiRainStopHandler() {
    this.emojiRainStop.emit();
  }
  ...
  componentDidLoad() {
    ...
    window.addEventListener(
      'resize',
      _e => {
        window.setTimeout(() => this._resizeWindow(), 50);
      },
      false,
    );
  ...
  _boundAnimate = this._animate.bind(this);
  /**
